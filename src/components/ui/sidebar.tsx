...
export const SidebarProvider: FC<SidebarProviderProps> = ({
  ...
  const setOpen = useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: state
...
export const SidebarContent: FC<ComponentProps<"div">> = ({
  ...
  return (
    <div
      ...
      className={cn(
        "no-scrollbar flex min-h-0 flex-1 flex-col gap-4 overflow-auto group-data-[collapsible=icon]:overflow-hidden",
        className,
      )}
      {...props}
    />
...
export const SidebarGroup: FC<ComponentProps<"div">> = ({
  ...
  return (
    <div
      ...
      data-sidebar="group"
      className={cn(
        "relative flex w-full min-w-0 flex-col gap-3 p-2",
        className,
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-all duration-200 ease-out hover:bg-sidebar-accent/60 hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent/80 active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-active:bg-sidebar-accent/20 data-active:font-medium data-active:text-primary data-open:hover:bg-sidebar-accent/60 data-open:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default:
...
export const SidebarMenuSub: FC<ComponentProps<"ul">> = ({
  ...
  return (
    <ul
      ...
      className={cn(
        "mx-3.5 flex min-w-0 translate-x-px flex-col gap-1.5 border-sidebar-border border-l px-2.5 py-2 group-data-[collapsible=icon]:hidden",
        className,
      )}
      {...props}
    />
...
export const SidebarMenuSubButton: FC<SidebarMenuSubButtonProps> = ({
  ...
  return useRender({
    ...
    props: mergeProps<"a">(
      {
        className: cn(
          "text-sidebar-foreground ring-sidebar-ring transition-all duration-200 ease-out hover:bg-sidebar-accent/60 hover:text-sidebar-accent-foreground active:bg-sidebar-accent/80 active:text-sidebar-accent-foreground [&>svg]:text-sidebar-accent-foreground data-active:bg-sidebar-accent/20 data-active:text-primary h-7 gap-2 rounded-md px-2 focus-visible:ring-2 data-[size=md]:text-sm data-[size=sm]:text-xs [&>svg]:size-4 flex min-w-0 -translate-x-px items-center overflow-hidden outline-hidden group-data-[collapsible=icon]:hidden disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:shrink-0",
          className,
        ),
      },
      props,
